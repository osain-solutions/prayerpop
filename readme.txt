...
* Let visitors message your team through a simple classic chat
* Reply from a shared WordPress chat inbox
...
* Chat uses a secure, HTTP-only browser cookie to reconnect a visitor to their conversation. The cookie contains a random token; only a one-way hash is stored in the database.

= Email notifications =

* Chat can email the configured site recipient when a visitor sends a message.
* Chat can email the visitor at the address they provide when an administrator replies.
...
* Plugin options, Chat tables, and scheduled cron hooks are removed on uninstall unless PrayerPop Pro is still installed and uses the shared data.
...
* Reorganized Free settings into clear task-based tabs, with global settings search and searchable, collapsible Language & Text groups.
